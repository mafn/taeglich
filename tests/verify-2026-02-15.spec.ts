...
async function waitForPlayerTurnOrResult(
  page: import("@playwright/test").Page,
) {
  await expect
    .poll(
      async () =>
        ((await page.locator("#gameStatus").textContent()) || "").trim(),
      {
        timeout: 5000,
      },
    )
...
test("2026-02-15 new game resets board to 3x3", async ({ page }) => {
  ...
  await expect
    .poll(async () => await page.locator("#grid .ttt-cell").count(), {
      timeout: 3000,
    })
    .toBeGreaterThan(9);
...
test("2026-02-15 honors XO unlock from sessionStorage", async ({ page }) => {
  await page.goto("/");
  await page.evaluate(() =>
    window.sessionStorage.setItem("taeglich:2026-02-15:xo", "1"),
  );
  await page.goto("/d/2026-02-15");
  await page.getByRole("button", { name: "Open T-TicTac app" }).click();
  await expect(page.locator("#appWindow")).toBeVisible();
  ...
  const xoBefore = await page
    .locator('#grid .ttt-cell[data-mark="XO"]')
    .count();
  await oCell.click({ force: true });
  await expect
    .poll(
      async () => await page.locator('#grid .ttt-cell[data-mark="XO"]').count(),
      {
        timeout: 3000,
      },
    )
    .toBeGreaterThan(xoBefore);
  const xoAfterFirst = await page
    .locator('#grid .ttt-cell[data-mark="XO"]')
    .count();
  ...
  expect(
    await page.evaluate(() =>
      window.sessionStorage.getItem("taeglich:2026-02-15:xo"),
    ),
  ).toBe("1");
});
...
test("2026-02-15 player XO unlock works and window shrinks on reset", async ({
  page,
}) => {
  await openGame(page);
...
test("2026-02-15 arrow navigation works in the grid", async ({ page }) => {
  ...
  await expect(
    page.locator('#grid .ttt-cell[data-row="0"][data-col="1"]'),
  ).toBeFocused();

  await page.keyboard.press("ArrowDown");
  await page.waitForTimeout(50);
  await expect(
    page.locator('#grid .ttt-cell[data-row="1"][data-col="1"]'),
  ).toBeFocused();

  await page.keyboard.press("ArrowLeft");
  await page.waitForTimeout(50);
  await expect(
    page.locator('#grid .ttt-cell[data-row="1"][data-col="0"]'),
  ).toBeFocused();

  await page.keyboard.press("ArrowUp");
  await page.waitForTimeout(50);
  await expect(
    page.locator('#grid .ttt-cell[data-row="0"][data-col="0"]'),
  ).toBeFocused();
  ...
  await page.waitForTimeout(50);
  await expect(
    page.locator('#grid .ttt-cell[data-row="0"][data-col="2"]'),
  ).toBeFocused();

  await page.keyboard.press("ArrowUp");
  await page.waitForTimeout(50);
  await expect(
    page.locator('#grid .ttt-cell[data-row="2"][data-col="2"]'),
  ).toBeFocused();
});
